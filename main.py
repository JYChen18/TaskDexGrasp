...
if __name__ == "__main__":
    ...
    config = {
        "miu_coef": [0.2, 0.],
        "obj_gravity_center": torch.tensor([[0.,0.,0.]]),
        "obj_obb_length": torch.tensor([0.1]),
        "enable_density": True,
        "tensor_args": tensor_args,
    }
    tdg_energy = TDGEnergy(**config)
    
    obj_pos = tensor_args.to_device([[[0.1, 0,0], [-0.1,0,0]]]) # [b, p, 3]
    obj_normal = tensor_args.to_device([[[-1., 0,0], [1,0,0]]]) # [b, p, 3]
    gras_energy = tdg_energy.forward(obj_pos, obj_normal)
    print("good", gras_energy)
    
    obj_pos = tensor_args.to_device([[[0.1, 0,0], [-0.1,0,0]]])
    obj_normal = tensor_args.to_device([[[-1., 0,0], [0,1,0]]])
    gras_energy = tdg_energy.forward(obj_pos, obj_normal)
    print("bad", gras_energy)
    
    obj_pos = tensor_args.to_device([[[0.1, 0,0], [-0.1,0,0], [-0.1,0,0]]]) # [b, p, 3]
    obj_normal = tensor_args.to_device([[[-1., 0,0], [1,0,0], [1,0,0]]]) # [b, p, 3]
    gras_energy = tdg_energy.forward(obj_pos, obj_normal)
    print("good only with enable_density=True", gras_energy)
